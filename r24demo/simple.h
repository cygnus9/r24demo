...
class Simple {
public:
	void init(const char* vertex_code, const char* shader_code, GLuint vertex_buffer, int vertex_count, const Geometry::attrib_defs* vertex_defs) {
	...
	void render() {
		...
		err = glGetError();
		Geometry::render(GL_QUADS, m_vertex_count, 2);
		err = glGetError();
...
public:
	...
	Quad() : Simple() {
		...
		static const Geometry::attrib_defs vertex_defs[] = { {"x", 1}, {nullptr} };
